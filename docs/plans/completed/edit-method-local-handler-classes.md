...
| `compare/00-feature-matrix.md` | Update `EditSource (surgical)` row — note local-class handler support |
| `docs_page/roadmap.md` | Mark PR-D ("edit_method local handler") as completed |
| `docs/plans/completed/rap-onprem-agent-gap-closure.md` | Already-completed adjacent plan; no changes needed but referenced |
...
6. **Eclipse evidence followed.** Per `compare/eclipse-adt/api/05-lock-create-update-transport.md`
...
- [ ] In `compare/00-feature-matrix.md` find the `EditSource (surgical)` row
...
- [ ] Confirm `docs/plans/completed/rap-onprem-agent-gap-closure.md` is still
...
- [ ] Smoke-test the ZBP_DM_PROJECT case from RUN-NOTES Run 3 manually:
      ...
      body changed.
- [ ] Move this plan from `docs/plans/edit-method-local-handler-classes.md` to
      `docs/plans/completed/edit-method-local-handler-classes.md`.
