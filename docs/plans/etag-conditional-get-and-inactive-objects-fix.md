...
- [ ] In `src/cache/caching-layer.ts:62-77`, replace the `getSource` implementation. The new signature: `async getSource(objectType: string, objectName: string, fetcher: (ifNoneMatch?: string) => Promise<{ source: string; etag?: string; notModified: boolean; statusCode: number }>, opts?: { version?: 'active' | 'inactive' }): Promise<{ source: string; hit: boolean; revalidated: boolean }>`.
  ...
  - Behaviour:
    ...
    2. If cache miss → call `fetcher(undefined)` (no `If-None-Match`) → store result via `cache.putSource(type, name, source, { version, etag })` → return `{ source, hit: false, revalidated: false }`.
    3. If cache hit and entry has no etag → call `fetcher(undefined)` (can't do conditional fetch) → if response body matches cached (or just trust the new response): replace cache, return `{ source: result.source, hit: false, revalidated: false }`. Treat as a miss because the cache had no validator.
    4. If cache hit with etag → call `fetcher(cached.etag)`:
       - If `result.notModified` (304): return `{ source: cached.source, hit: true, revalidated: true }`. Optionally update `cachedAt` for stats by re-`putSource`-ing with the same body+etag (negligible cost).
...
- [ ] In `tests/unit/cache/caching-layer.test.ts:28-70`, replace the `source caching` describe block with new tests reflecting the conditional-GET flow (~7 tests):
...
- [ ] Create `docs/caching.md`. Structure:
  ...
  - **What invalidates**: SAPWrite mutations call `cachingLayer.invalidate(type, name)` (active version). gCTS sync calls `invalidate` per-synced-object. External activations are caught by the next conditional GET — no manual invalidation needed.
