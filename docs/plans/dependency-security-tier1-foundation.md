...
**✅ Completed in [arc-1#236](https://github.com/marianfoo/arc-1/pull/236)** (merged 2026-05-08). PR-time `actions/dependency-review-action@v4` running on every PR with `fail-on-severity: high`, `comment-summary-in-pr: on-failure`, and a deny-list for copyleft licenses (GPL-2.0/3.0, AGPL-3.0, LGPL-2.1/3.0). Self-review caught a real bug: the first attempt set both `allow-licenses` and `deny-licenses`, which the action rejects with `"You cannot specify both"` — fixed in the same PR before merge. Ralphex should skip this task — proceed to Task 5.
...
**⏸️ DEFERRED — CodeQL is running on GitHub's Default Setup instead.** The maintainer enabled CodeQL Default Setup in repo Settings → Advanced Security on 2026-05-08; it has been producing SARIF findings to the Security tab continuously since then (12 findings on first run, 5 closed by [arc-1#229](https://github.com/marianfoo/arc-1/pull/229) workflow-permissions hardening, 7 HIGH remaining for separate triage). Default vs Advanced Setup are mutually exclusive — adding `.github/workflows/codeql.yml` while Default Setup is on would be a no-op (Default wins).
...
- [x] Push the branch. Confirm the workflow runs and uploads results to `https://github.com/marianfoo/arc-1/security/code-scanning`. First-run CodeQL surfaces a baseline of findings — these are not regressions; review them and triage in a follow-up PR or dismiss with documented rationale.
- [x] Run `npm test` — all tests must pass.
...
**✅ Completed in [arc-1#236](https://github.com/marianfoo/arc-1/pull/236)** (merged 2026-05-08). `aquasecurity/trivy-action@v0.36.0` (SHA-pinned) added to both workflows. `docker.yml` runs non-gating (`exit-code: 0`) on dev pushes to surface findings without blocking iteration. `release.yml` runs gating (`exit-code: 1`) so a release fails on HIGH/CRITICAL CVEs and customers pulling `:vX.Y.Z` get a CVE-clean image. Both upload SARIF to the GitHub Security tab via `github/codeql-action/upload-sarif@v4`. New `security-events: write` permission added to both `build`/`publish-docker` jobs. Ralphex should skip this task — proceed to Task 7.
...
**✅ Completed in [arc-1#236](https://github.com/marianfoo/arc-1/pull/236)** (merged 2026-05-08). All six third-party actions are SHA-pinned with trailing `# <tag>` comments. Concrete SHAs in main:
...
GitHub-owned `actions/*` and `github/*` deliberately stay tag-pinned. Note: `googleapis/release-please-action` was bumped from v4 → v5 by Dependabot's actions group ([#234](https://github.com/marianfoo/arc-1/pull/234)) before this task ran, so the SHA pinning landed on v5. Ralphex should skip this task — proceed to Task 8.
...
**✅ Completed in [arc-1#236](https://github.com/marianfoo/arc-1/pull/236)** (merged 2026-05-08). All five doc artifacts updated:
...
**✅ Completed — all Tier 1 tasks landed across PRs [#227](https://github.com/marianfoo/arc-1/pull/227), [#229](https://github.com/marianfoo/arc-1/pull/229), [#234](https://github.com/marianfoo/arc-1/pull/234), [#235](https://github.com/marianfoo/arc-1/pull/235), [#236](https://github.com/marianfoo/arc-1/pull/236)** (all merged 2026-05-08). Task 5 (custom CodeQL workflow) intentionally deferred — Default Setup is in active use. Verification signal: PR #236's CI run on the merge commit showed `mta-validate`, `test (22)`, `test (24)`, `dependency-review`, `CodeQL`, `Analyze (actions)`, and `Analyze (javascript-typescript)` all green; `npm audit --audit-level=high` baseline is 0 vulnerabilities; 2,524 unit tests pass on Node 22 + 24. The `integration` and `e2e` jobs failed architecturally (Dependabot/PR-secret unavailability pattern, unrelated to PR contents).
