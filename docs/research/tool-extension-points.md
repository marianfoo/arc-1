...
## 14. Case study: integrating dassian-adt v2.0 as an ARC-1 extension
...
### 14.2 Tool inventory (39 tools, 12 handlers)
...
### 14.3 Coverage vs ARC-1's 12 intent tools
...
### 14.4 The hard architectural mismatch: HTTP client identity
...
### 14.5 Feature-by-feature: can a plugin author build this under FEAT-61 today?
...
| `askClaude(systemPrompt, userMessage, maxTokens?)` — handler asks the LLM a sub-question | `Server.createMessage()` (sampling) | 🟡 | **Add `ctx.sampling?: (systemPrompt, userMessage, maxTokens?) => Promise<string>`** to §7. Already noted in §14.6 (was Open Question 11) — promote to Phase-1 yes. |
...
### 14.6 Concrete updates the case study makes to the FEAT-61 design
...
### 14.7 Summary: how a plugin author maps each dassian-adt feature

The full integration model under FEAT-61 with the §14.6 additions applied:
...
* **Mid-flow user interaction (elicitation, progress notify, internal sampling):** **buildable only after §7 ToolContext gains the three new optional capabilities listed in §14.6.** This is the change the case study most strongly recommends for Phase 1.
