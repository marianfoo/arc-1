---
name: drawio
description: Always use when user asks to create, generate, draw, or design a diagram, flowchart, architecture diagram, ER diagram, sequence diagram, class diagram, network diagram, mockup, wireframe, UI sketch, or an SAP BTP / SAP Architecture Center / Fiori / subaccount / Cloud Connector / on-prem SAP / S/4HANA landscape / solution diagram, or mentions draw.io, drawio, drawoi, .drawio files, or diagram export to PNG/SVG/PDF.
---
...
## SAP Architecture Center style (BTP, Fiori, on-prem SAP)

If the user's request involves SAP, SAP BTP, Cloud Foundry, Kyma, subaccount, Fiori, S/4HANA, Cloud Connector, or matches the visual style of https://architecture.learning.sap.com — **before generating XML**, read the SAP style reference:

```
.claude/skills/drawio/references/btp-sap-style.md
```

That file has the canonical palette, typography, shape defaults, layout idiom, and the copy-and-edit workflow using the bundled reference templates and icon library under `.claude/skills/drawio/assets/`. Do not improvise SAP styling from memory — load the reference.

After writing the diagram, validate it:

```
python3 .claude/skills/drawio/scripts/validate_drawio.py <file.drawio>
```

Export with `-b 10 -s 2` (2× scale, 10 px border) for crisp embedding in MkDocs.
