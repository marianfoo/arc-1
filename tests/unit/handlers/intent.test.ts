...
ENDCLASS.`;
  ...
  // ─── SAPWrite RAP feature guard ──────────────────────────────────

  describe('SAPWrite RAP feature guard', () => {
    const rapUnavailableFeatures = {
      hana: { available: false },
      abapGit: { available: false },
      rap: { available: false },
      amdp: { available: false },
    ...
    const rapAvailableFeatures = {
      hana: { available: true },
      abapGit: { available: true },
      rap: { available: true },
      amdp: { available: true },
      ui5: { available: true },
    ...
    it('SAPWrite create DDLS returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'create',
        type: 'DDLS',
        name: 'ZI_TEST',
        source: 'define root view entity ZI_TEST {}',
        package: '$TMP',
      });
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('RAP/CDS feature is not available');
      expect(result.content[0]!.text).toContain('DDLS');
    });

    it('SAPWrite create BDEF returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'create',
        type: 'BDEF',
        name: 'ZI_TEST',
        source: 'managed implementation in class zbp_i_test;',
        package: '$TMP',
      });
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('RAP/CDS feature is not available');
    ...
    it('SAPWrite create SRVD returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'create',
        type: 'SRVD',
        name: 'ZSD_TEST',
        source: 'define service ZSD_TEST {}',
        package: '$TMP',
      });
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('RAP/CDS feature is not available');
      expect(result.content[0]!.text).toContain('SRVD');
    });

    it('SAPWrite create DDLX returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'create',
        type: 'DDLX',
        name: 'ZC_TEST',
        source: '@Metadata.layer: #CUSTOMER annotate view ZC_TEST with {}',
        package: '$TMP',
      });
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('RAP/CDS feature is not available');
      expect(result.content[0]!.text).toContain('DDLX');
    });

    it('SAPWrite batch_create with RAP types returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'batch_create',
        package: '$TMP',
        objects: [
          { type: 'DDLS', name: 'ZI_TEST', source: 'define root view entity ZI_TEST {}' },
          { type: 'BDEF', name: 'ZI_TEST', source: 'managed;' },
        ],
      });
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('RAP/CDS feature is not available');
    ...
    it('SAPWrite update DDLS returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'update',
        type: 'DDLS',
        name: 'ZI_TEST',
        source: 'define root view entity ZI_TEST { key id : sysuuid_x16; }',
      });
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('RAP/CDS feature is not available');
    });

    it('SAPWrite delete DDLS returns error when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'delete',
        type: 'DDLS',
    ...
    it('SAPWrite create DDLS succeeds when rap.available=true', async () => {
      setCachedFeatures(rapAvailableFeatures);
      mockFetch.mockResolvedValue(mockResponse(200, '<xml>ok</xml>', { 'x-csrf-token': 'T' }));
      const config = { ...DEFAULT_CONFIG, lintBeforeWrite: false };
      const result = await handleToolCall(createClient(), config, 'SAPWrite', {
        action: 'create',
    ...
    it('SAPWrite create DDLS succeeds when features not probed', async () => {
      resetCachedFeatures(); // ensure no cached features
      mockFetch.mockResolvedValue(mockResponse(200, '<xml>ok</xml>', { 'x-csrf-token': 'T' }));
      const config = { ...DEFAULT_CONFIG, lintBeforeWrite: false };
      const result = await handleToolCall(createClient(), config, 'SAPWrite', {
        action: 'create',
    ...
    it('SAPWrite create PROG is not affected by rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      mockFetch.mockResolvedValue(mockResponse(200, '<xml>ok</xml>', { 'x-csrf-token': 'T' }));
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'create',
    ...
    it('SAPWrite batch_create with only non-RAP types succeeds when rap.available=false', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      mockFetch.mockResolvedValue(mockResponse(200, '<xml>ok</xml>', { 'x-csrf-token': 'T' }));
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'batch_create',
    ...
    it('error message includes probe re-check hint', async () => {
      setCachedFeatures(rapUnavailableFeatures);
      const result = await handleToolCall(createClient(), DEFAULT_CONFIG, 'SAPWrite', {
        action: 'create',
        type: 'DDLS',
      ...
      expect(result.isError).toBe(true);
      expect(result.content[0]!.text).toContain('SAPManage(action="probe")');
      expect(result.content[0]!.text).toContain('/sap/bc/adt/ddic/ddl/sources');
    });
  });
