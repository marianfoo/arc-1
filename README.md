...
- **HTTP security headers (helmet) on by default** — HSTS, CSP, X-Frame-Options, COOP `same-origin-allow-popups` (so OAuth popups still work), CORP, X-Content-Type-Options. No flag to disable.
