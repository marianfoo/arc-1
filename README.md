...
- **Layered rate limiting** — three layers out of the box: per-IP OAuth/`/mcp` edge (default 20/min/IP), per-user MCP quota (default 60/min/user), server-wide SAP-bound semaphore (default 10). Honors `Retry-After` on 429/503 from SAP / BTP gateways. Two operator env vars; per-endpoint OAuth ceilings are constants in code. Closes CodeQL alert `js/missing-rate-limiting`. See the [Rate Limiting Guide](https://marianfoo.github.io/arc-1/rate-limiting/)
