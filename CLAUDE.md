...
| Add function-group (FUGR) and function-module (FUNC) write (issue #250) | `src/handlers/intent.ts` (FUNC URL pre-resolution branch in `handleSAPWrite` ~line 3048; new `case 'FUGR'` and `case 'FUNC'` in `buildCreateXml` ~line 2575; `FUGR`/`FUNC` mappings in `vendorContentTypeForType` + `needsVendorContentType`; FUNC URL pre-resolution branches in `handleSAPActivate` single + batch paths; `stripFmParamCommentBlock` helper near `escapeXml`), `src/handlers/schemas.ts` (`'FUGR'` in `SAPWRITE_TYPES_ONPREM`; `group` field on `SAPWriteSchema` + `SAPWriteSchemaBtp`), `src/handlers/tools.ts` (`'FUGR'` in `SAPWRITE_TYPES_ONPREM`; `group` JSON schema property; description caveat about FM signature management not being supported). The FUGR write routes through the existing `objectBasePath('FUGR')` fallback (returns `/sap/bc/adt/functions/groups/`); FUNC bypasses `objectBasePath('FUNC')` entirely (which deliberately throws — keep that throw intact). FM parameter signature management (IMPORTING/EXPORTING/EXCEPTIONS) is **explicitly out of scope** — managed via SAPGUI/SE37 or Eclipse. SAPGUI-style `*"…"*` parameter comment blocks in source are auto-stripped before PUT (SAP rejects them with `FUNC_ADT028`). Tests: `tests/unit/handlers/intent.test.ts` (buildCreateXml + SAPWrite routing + stripFmParamCommentBlock), `tests/unit/handlers/schemas.test.ts`, `tests/unit/handlers/tools.test.ts`, `tests/integration/fugr-func.integration.test.ts`, `tests/e2e/func-write.e2e.test.ts`. See `docs/plans/completed/add-func-fugr-create-write.md`. |
